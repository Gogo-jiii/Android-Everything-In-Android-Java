...
public class DatabaseManager extends SQLiteOpenHelper {
    ...
    public UsersModel getUser(int id) {
        ...
        UsersModel model;
        try (Cursor cursor = database.query(USERS_TABLE_NAME, new String[]{USERS_COLUMN_ID,
                        USERS_COLUMN_NAME},
                USERS_COLUMN_ID + "=?",
            ...
            model = new UsersModel(Integer.parseInt(Objects.requireNonNull(cursor).getString(0)),
                    cursor.getString(1));
        }
    ...
    public List<UsersModel> getAllUsers() {
        ...
        SQLiteDatabase database = this.getReadableDatabase();
        try (Cursor cursor = database.rawQuery("select * from users", null)) {
    ...
    public int getNumberOfRows() {
        SQLiteDatabase database = this.getReadableDatabase();
        return (int) DatabaseUtils.queryNumEntries(database, USERS_TABLE_NAME);
    }
